              ...
              <div class="row justify-center">
                <div class="col-10">
                  <q-btn
                    ...
                    :color="btn.color"
                    @click="clkOpenPendingSolicitation(btn.callback)"
                    outline
                    no-caps
                    unelevated
                  />
...
export default defineComponent({
  ...
  methods: {
    clkOpenUserDetail(e, r){
      if(r.status && r.status.status === 'active'){
        const userId = r._id
        this.$router.push('/users/userDetail?userId=' + userId)
        return
      }else{ this.$q.notify('Usuário não ativo') }
    },
    clkOpenPendingSolicitation(callback){
    ...
    refuseFunctionSolicitation(){
      const opt = {
        route: "/desktop/adm/refuseFunctionSolicitation",
        body: {
          functionSolicitationId: this.dialogOpenSolicitation.data._id,
          organismId: this.dialogOpenSolicitation.data.organismId,
        },
